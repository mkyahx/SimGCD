...
class AsymmetricMaskModel(nn.Module):
    """SimGCD encoder with a global first view and a mask-filtered second view.

    During training, outputs are ordered as ``[global(view 1); foreground(view 2)]``
    so the original two-view SimGCD losses can be reused unchanged. During evaluation,
    the foreground branch is used alone to match the mask-guided training target.
    """
    ...
    def __init__(
        ...
        super().__init__()
        self.mask_encoder = MaskForegroundGLSimModel(
            backbone=backbone,
            head=nn.Identity(),
            feat_dim=feat_dim,
