...
# This must be configured before importing torch or creating a CUDA context.
...
def build_mask_datasets(train_transform, test_transform, args):
    ...
    if args.dataset_name not in supported_mask_datasets:
        raise NotImplementedError(
            f"The asymmetric mask entrypoint supports {sorted(supported_mask_datasets)}, "
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            if len(images) != 2 or len(patch_mask) != 2:
                raise ValueError("Asymmetric mask training requires exactly two image/mask views.")
            ...
            with torch.cuda.amp.autocast(fp16_scaler is not None):
                student_proj, student_out = student((images[0], images[1], patch_mask[1]))
...
def seed_worker(worker_id):
    """Synchronize Python, NumPy, and Torch RNGs inside each DataLoader worker."""
...
def set_strict_seed(seed):
    if not torch.cuda.is_available():
        raise RuntimeError("train_glsim_mask_repro.py requires a CUDA-enabled PyTorch runtime.")

    python_hash_seed = os.environ.get("PYTHONHASHSEED")
    if python_hash_seed != str(seed):
        raise RuntimeError(
            "Strict reproducibility requires PYTHONHASHSEED to equal --seed before Python starts. "
            "Launch through scripts/run_glsim_mask_repro.sh."
        )
...
if __name__ == "__main__":
    ...
    init_experiment(args, runner_name=["simgcd-asymmetric-mask-repro"])
    ...
    train_loader_generator.manual_seed(args.seed + 1)
    eval_loader_generator = torch.Generator()
    eval_loader_generator.manual_seed(args.seed + 2)
